...
class TaskRepository:
    ...
    async def create_task(
        ...
        logger.info(f"Created task: task_id={task.task_id}, expires_at={expires_at.isoformat()}")
    ...
    async def cleanup_expired_tasks(self) -> int:
        ...
        async with self._lock:
            for task_id, task in list(self._tasks.items()):  # Use list to allow modification during iteration
                if task.expires_at and task.status == "in_progress":
                    if now >= task.expires_at:
                        task.status = "failed"
                        task.error = {"message": "Task deadline exceeded", "type": "DeadlineExceeded"}
                        cleaned_count += 1
                        logger.info(f"Marked task {task_id} as failed due to deadline expiration")
