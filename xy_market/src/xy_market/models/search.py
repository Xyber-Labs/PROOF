"""Search models for SearchEngineBK integration."""
...
class SearchRequest(BaseModel):
    """Search request from Buyer to SearchEngineBK.
    """

    task_description: str = Field(..., description="Task description for semantic search")
    tags: list[str] | None = Field(default=None, description="Optional tags for targeted search")
    ...
    def validate_budget_range(cls, v: tuple[float, float] | None) -> tuple[float, float] | None:
        ...
        if v is not None:
            ...
            if max_budget < min_budget:
                raise ValueError(f"budget_range max ({max_budget}) must be >= min ({min_budget})")
            if min_budget < 0:
...
class SellerProfile(BaseModel):
    """Seller profile returned by SearchEngineBK."""
...
class SearchResponse(BaseModel):
    """Search response from SearchEngineBK.
    
    Synchronous response containing relevant sellers.
    """

    sellers: list[SellerProfile] = Field(..., description="List of relevant Seller profiles")
    search_id: str | None = Field(default=None, description="Optional ID for tracking the search request")
