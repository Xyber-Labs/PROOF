...
class ExecutionResult(BaseModel):
    ...
    buyer_secret: str = Field(
        ..., description="Secret UUID for polling task status (assigned by Seller)"
    )
    ...
    data: dict = Field(
        ...
        description="Result data (only populated when status='done')",
    )
    ...
    deadline_at: str | None = Field(
        default=None,
        description="Deadline timestamp (ISO 8601) - when task must be completed by",
    )
