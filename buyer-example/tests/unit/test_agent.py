...
def mock_buyer_sdk():
    ...
    sdk = MagicMock()
    sdk.search_sellers = AsyncMock()
    sdk.execute_task = AsyncMock()
    sdk.poll_task_status = AsyncMock()
    return sdk
...
def test_agent_initializes_llm(mock_buyer_sdk, mock_settings, mock_llm):
    """Test that agent initializes LLM."""
    with patch("buyer_example.agent.get_settings", return_value=mock_settings), \
        ...
        agent = BuyerAgent(mock_buyer_sdk)

        assert agent.llm is mock_llm


def test_agent_binds_tools_to_llm(mock_buyer_sdk, mock_settings, mock_llm):
...
def test_agent_builds_graph(mock_buyer_sdk, mock_settings, mock_llm):
...
def test_agent_node_adds_system_prompt(mock_buyer_sdk, mock_settings, mock_llm):
...
def test_agent_node_skips_system_prompt_if_exists(mock_buyer_sdk, mock_settings, mock_llm):
...
def test_should_continue_returns_end_for_no_tool_calls(mock_buyer_sdk, mock_settings, mock_llm):
...
def test_should_continue_returns_continue_for_tool_calls(mock_buyer_sdk, mock_settings, mock_llm):
...
def test_should_continue_returns_end_for_empty_messages(mock_buyer_sdk, mock_settings, mock_llm):
...
async def test_process_message_returns_result(mock_buyer_sdk, mock_settings, mock_llm):
...
async def test_process_message_extracts_ai_response(mock_buyer_sdk, mock_settings, mock_llm):
...
async def test_process_message_builds_conversation_history(mock_buyer_sdk, mock_settings, mock_llm):
