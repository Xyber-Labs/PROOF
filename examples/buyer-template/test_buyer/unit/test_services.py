...
def test_service_initializes_marketplace_client(
    ...
    with (
        ...
        patch("buyer_example.services.MarketplaceClient") as mock_client_class,
    ):
...
def test_service_initializes_without_wallet(mock_settings, mock_buyer_x402_settings):
    ...
    with (
        ...
        # MarketplaceClient should be created with http_client=None
...
def test_agent_property_lazy_initialization(
    mock_settings, mock_buyer_x402_settings, mock_marketplace_client, mock_buyer_agent
):
...
def test_agent_property_returns_same_instance(
    mock_settings, mock_buyer_x402_settings, mock_marketplace_client, mock_buyer_agent
):
...
async def test_process_user_request_calls_agent(
    mock_settings, mock_buyer_x402_settings, mock_marketplace_client, mock_buyer_agent
):
    ...
    with (
        ...
        mock_buyer_agent.process_message.assert_called_once_with(
...
async def test_close_closes_marketplace_client(
    mock_settings, mock_buyer_x402_settings, mock_marketplace_client
):
    """Test that close() closes the MarketplaceClient."""
    with (
        patch("buyer_example.services.get_settings", return_value=mock_settings),
        patch(
            "buyer_example.services.get_buyer_x402_settings",
        ...
        mock_marketplace_client.close.assert_called_once()
