...
class X402Config(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="SELLER_TEMPLATE_X402_",
        env_file=".env",
...
class AppSettings(BaseSettings):
    """
    Application settings for the Seller Template.

    Configuration can be provided via environment variables using nested notation:

    # Server settings:
    SELLER_TEMPLATE_HOST=0.0.0.0
    SELLER_TEMPLATE_PORT=8000
    SELLER_TEMPLATE_EXECUTION_TIMEOUT_SECONDS=300
    """
...
class BuyerX402Settings(BaseSettings):
    """Settings for the seller to act as a buyer when connecting to other MCP servers."""
