...
class ExecutionService:
    ...
    def __init__(
        ...
        self.task_repository = TaskRepository(default_deadline_seconds=default_deadline_seconds)
    ...
    async def create_task(
        ...
        task = asyncio.create_task(
            self._execute_task_async(task_id, execution_request)
        )
    ...
    async def _execute_task_async(
        ...
        try:
            if not self.archivist_agent:
                raise RuntimeError("Agent not initialized. This should not happen - agent initialization should fail on startup.")
